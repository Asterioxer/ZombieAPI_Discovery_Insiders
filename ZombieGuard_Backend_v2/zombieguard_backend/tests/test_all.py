...
class TestURLScanner:
    ...
    def test_url_scanner_find_spec_url(self):
        from app.services.url_scanner import URLAPIScanner
        scanner = URLAPIScanner()
        html = """
        <html>
        <head>
            <script>
        ...
        """
        spec_url = scanner._find_swagger_spec_url_in_html(html, "https://test.com")
        assert spec_url == "https://test.com/v2/swagger.json"
        print("  ✅ _find_swagger_spec_url_in_html works correctly")

    def test_url_scanner_direct_spec_parsing(self):
        from app.services.url_scanner import URLAPIScanner
        scanner = URLAPIScanner()
        spec = {
            "swagger": "2.0",
        ...
        eps = scanner._parse_spec_dict(spec, "test_source")
        assert len(eps) == 1
        assert eps[0]["endpoint"] == "/api/v1/users"
        assert eps[0]["method"] == "GET"
        assert eps[0]["has_auth"] is True
        print("  ✅ _parse_spec_dict works correctly")
    ...
    def test_url_scanner_graceful_fail(self):
        ...
        req = URLScanRequest(url="http://localhost:99999")
    ...
    def test_api_scan_url_invalid(self):
        ...
        r = client.post("/api/scan/url", json={"url": "http://127.0.0.1:9999/invalid"})
